...
# обычный, промежуточный, встроенный режимы проекта
project_modes = {
    'c1': [3.2, 3.0, 2.8],
    'p1': [1.05, 1.12, 1.2],
    'c2': [2.5, 2.5, 2.5],
...
class MainWindow(QDialog):
    ...
    def calculate_project(self):
        ...
        pm_clean = round(self.PM(), 2)  # трудоемкость без учета планирования
        tm_clean = round(self.TM(), 2)  # время без учета планирования
        pm = round(pm_clean * 1.08, 2)  # трудоемкость с учетом планирования
        tm = round(tm_clean * 1.36, 2)  # время с учетом планирования
    ...
    def calculate_task1(self):
        ...
        col_headers = ['Обычный', 'Промежуточный', 'Встроенный']
        ...
        for cplx in [2]:
            for t in range(3):
                ...
                plt.suptitle(f'CPLX={levels[cplx]}')
                plt.subplot(2, 3, t + 1)
                plt.title(col_headers[t])
                line1, = plt.plot(names, y_acap_pm, 'blue', label='ACAP')
                line2, = plt.plot(names, y_aexp_pm, 'indigo', label='AEXP')
                line3, = plt.plot(names, y_pcap_pm, 'teal', label='PCAP')
                line4, = plt.plot(names, y_lexp_pm, 'magenta', label='LEXP')
                plt.tight_layout()
                plt.ylabel("Трудоемкость")
                plt.legend(handles=[line1, line2, line3, line4])

                plt.subplot(2, 3, t + 4)
                line1, = plt.plot(names, y_acap_tm, 'blue')
                line2, = plt.plot(names, y_aexp_tm, 'indigo')
                line3, = plt.plot(names, y_pcap_tm, 'teal')
                line4, = plt.plot(names, y_lexp_tm, 'magenta')
                # plt.plot(names, y_acap_tm, 'r', names, y_aexp_tm, 'g', names, y_pcap_tm, 'b', names, y_lexp_tm, 'y')
